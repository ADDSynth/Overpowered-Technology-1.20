...
public final class GuiLaserHousing extends GuiEnergyBase<TileLaserHousing, ContainerLaserHousing> {
  ...
  private static final Component required_energy_text = Component.translatable("gui.overpowered.laser_housing.required_energy");
  private static final Component current_energy_text  = Component.translatable("gui.overpowered.laser_housing.current_energy");
  private static final Component lasers_text          = Component.translatable("gui.overpowered.laser_housing.lasers");
  private static final Component distance_text        = Component.translatable("gui.overpowered.laser_housing.distance");
  ...
  @Override
  public final void render(GuiGraphics graphics, final int mouseX, final int mouseY, final float partialTicks){
  ...
  protected final void renderLabels(GuiGraphics graphics, int mouseX, int mouseY){
    draw_title(graphics);
    draw_text_left(graphics, lasers_text.getString()+": "+tile.number_of_lasers, 6, line_1);
    draw_text_right(graphics, distance_text.getString()+": ", text_box_x - 2, line_1);
    draw_energy_requirements(graphics);
    draw_text_right(graphics, energy_bar.getEnergyPercentage(), line_4);
    draw_energy_difference_center(graphics, line_5);
  }

  private final void draw_energy_requirements(final GuiGraphics graphics){
    ...
    if(Math.max(word_1_width, word_2_width) == word_1_width){
      draw_text_left(graphics, word_1, 6, line_2);
      draw_text_left(graphics, current_energy_text.getString()+":", 6, line_3);
      draw_text_right(graphics, current_energy, 6 + word_1_width, line_3);
    }
    else{
      draw_text_left(graphics, required_energy_text.getString()+":", 6, line_2);
      draw_text_right(graphics, required_energy, 6 + word_2_width, line_2);
      draw_text_left(graphics, word_2, 6, line_3);
    }
  }
