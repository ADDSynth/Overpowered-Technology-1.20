...
public final class IngredientWidget {
  ...
  public final void draw(final GuiGraphics graphics, int x, int y){
    if(stacks != null){
      if(stacks.length > 0){
        graphics.renderItem(stacks[index], x, y);
      }
    }
  }
  ...
  public final void drawTooltip(GuiGraphics graphics, Font font, final Screen screen, final int x, final int y, final int mouse_x, final int mouse_y){
    if(stacks != null){
      if(stacks.length > 0){
        GuiUtil.drawItemTooltip(graphics, font, screen, stacks[index], x, y, mouse_x, mouse_y);
      }
    }
  }
